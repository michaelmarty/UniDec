...
class Extract2DPlot(wx.Frame):
    def __init__(self, parent, datalist,config=None,yvals=None,directory=None,header=None,params=None,*args,**kwargs):
        wx.Frame.__init__(self, parent, title="2D Grid Extraction",size=(-1,-1))

        self.filemenu = wx.Menu()

        self.menuSaveFigPNG=self.filemenu.Append(wx.ID_ANY,"Save Figures as PNG","Save all figures as PNG in central directory")
        self.menuSaveFigPDF=self.filemenu.Append(wx.ID_ANY,"Save Figures as PDF","Save all figures as PDF in central directory")
    ...
    def makeplot(self):
        ...
        try:
            self.plot2.contourplot(dat,self.config,xlab="mass 1",ylab="mass 2",title=title,normflag=self.normflag,normrange=[0,1])
        except:
            self.plot2.clear_plot()
            print "Failed Plot2"
        try:
            self.plot1.plotrefreshtop(np.unique(self.m1grid),np.sum(self.igrid[i],axis=1),title,"mass 1","Total Intensity","",self.config,test_kda=False,nopaint=False)
        except:
    ...
    def makeplottotal(self):
        ...
        #self.config.cmap="jet"
        try:
            self.plot2.contourplot(dat,self.config,xlab="mass 1",ylab="mass 2",title="Total Extraction",normflag=0,normrange=[np.amin(grid),np.amax(grid)])
            ...
            print "Failed Plot2"
        try:
            self.plot1.plotrefreshtop(np.unique(self.m1grid),np.sum(grid,axis=1),"Total Projection","mass 1","Total Intensity","",self.config,test_kda=False,nopaint=False)
    ...
    def makeplotwap(self):
        ...
        dat=np.transpose([np.ravel(self.m1grid),np.ravel(self.m2grid),np.ravel(grid)])
        try:
            self.plot2.contourplot(dat,self.config,xlab="mass 1",ylab="mass 2",title="Weighted Average of Position",normflag=0,normrange=[np.amin(grid),np.amax(grid)])
        except:
            self.plot2.clear_plot()
            print "Failed Plot2"
        try:
            self.plot1.plotrefreshtop(np.unique(self.m1grid),np.average(grid,axis=1),"Average Projection","mass 1","Average Position","",self.config,test_kda=False,nopaint=False)
