...
def MinFreeError(kds, data, weights, kdargs):
    out = [MinFree(kdargs.pconc[i], kdargs.lconc[i], kdargs.ureact, kdargs.nprottab, kdargs.nligtab, kdargs.paths, kds,
                   kdargs.pfrees[i], kdargs.lfrees[i], kdargs.nfactors) for i in xrange(0, len(kdargs.lconc))]
    out = np.array(out)
    kdargs.pfrees = out[:, 0]
    kdargs.lfrees = out[:, 1]
    errors = np.ravel([GetError(data[:, i], kdargs.pfrees[i], kdargs.lfrees[i], kdargs.ureact, kdargs.nprottab,
                       ...
                       i in xrange(0, len(kdargs.lconc))])
    return errors
        ...
        for i in xrange(0, len(kdargs.lconc)):
            out = [
                MinFree(kdargs.pconc[i], kdargs.lconc[i], kdargs.ureact, kdargs.nprottab, kdargs.nligtab, kdargs.paths,
                        kds, kdargs.pfrees[i], kdargs.lfrees[i], kdargs.nfactors) for i in xrange(0, len(kdargs.lconc))]
            out = np.array(out)
            pfrees = out[:, 0]
            lfrees = out[:, 1]
            degenkd = []
...
class KDmodel:
    ...
    def FindBestModel(self, fixedprotmodel, fixedligmodel):
        ...
        compares = np.array([CompareModels(float(fitssorted[i][0]), float(fitssorted[i + 1][0]),
                                           ...
                                           len(np.ravel(self.data)), 0.99) for i in xrange(0, len(fits) - 1)])
        test = 0
        print "P Values: "
        print compares
    ...
    def MakeFitGrid(self):
        ...
        self.fitgrid = np.transpose(
            np.array([safedivide(np.array(out2[i]), np.sum(out2[i])) for i in xrange(0, len(kdargs.lconc))]))
    ...
    def Bootstrap(self, std, numpts):
        ...
        hists = [np.reshape(np.histogram(
            ...
            bins=range(0, len(np.ravel(self.data)) + 1))[0], np.shape(self.data)) for i in xrange(0, numpts)]
        ...
        try:
            ...
            workers = [multiprocessing.Process(target=BootMinWorker, args=(queue, results_queue)) for i in
                       xrange(0, multiprocessing.cpu_count())]
            for p in workers:
                p.start()
            count = 0
            while True:
...
if __name__ == "__main__":
    ...
    if switch == 0:
        ...
        lconc = data[1]
        data = data[2:]
        header = string.split(file_name, ".")[0]
        nprot = 1
        nlig = 4
        maxsites = 4
