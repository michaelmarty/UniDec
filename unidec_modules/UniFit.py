...
def sortarray(a):
    shape=a.shape
    a=[list(a[i]) for i in xrange(0,shape[0])]
...
def GetError(data,pfree,lfree,ureact,prottab,ligtab,paths,kds,weights,nodelist,nfactors):
    ...
    sum=np.sum(extract)
    if sum>0:
        return (weights*(extract/sum-data))**2
    else:
...
def draw_graph_structure(graph1,graph2,kdmap,ax=None):
    ...
    # add nodes
    for node in nodes:
        split1=string.split(node,"L")
        split2=string.split(split1[0],"P")
...
def draw_graph(graph1,graph2,kds,errors,kdmap,header,ax=None):
    ...
    for i in range(0,len(graph1)):
        edge=graph1[i]
        string1=u"KD%d=%.3g" %(kdmap[i],kds[kdmap[i]])
...
def MinFreeError(kds,data,weights,kdargs):
    out=[MinFree(kdargs.pconc[i],kdargs.lconc[i],kdargs.ureact,kdargs.nprottab,kdargs.nligtab,kdargs.paths,kds,kdargs.pfrees[i],kdargs.lfrees[i],kdargs.nfactors) for i in xrange(0,len(kdargs.lconc))]
    out=np.array(out)
    kdargs.pfrees=out[:,0]
    kdargs.lfrees=out[:,1]
    errors=np.ravel([GetError(data[:,i],kdargs.pfrees[i],kdargs.lfrees[i],kdargs.ureact,kdargs.nprottab,kdargs.nligtab,kdargs.paths,kds,weights[:,i],kdargs.nodelist,kdargs.nfactors) for i in xrange(0,len(kdargs.lconc))])
    return errors

def GetDegenKD(kds,kdargs):
    if len(kdargs.degen>0):
        dktot=[]
        for i in xrange(0,len(kdargs.lconc)):
            out=[MinFree(kdargs.pconc[i],kdargs.lconc[i],kdargs.ureact,kdargs.nprottab,kdargs.nligtab,kdargs.paths,kds,kdargs.pfrees[i],kdargs.lfrees[i],kdargs.nfactors) for i in xrange(0,len(kdargs.lconc))]
            out=np.array(out)
            pfrees=out[:,0]
            lfrees=out[:,1]
            degenkd=[]
            grid=MakeGrid(pfrees[i],lfrees[i],kdargs.ureact,kdargs.nprottab,kdargs.nligtab,kdargs.paths,kds,kdargs.nfactors)[0]
            for react in kdargs.degen[:,1]:
                degenkd.append(grid[react[0,0],react[0,1]]*grid[react[1,0],react[1,1]]/grid[react[2,0],react[2,1]])
...
class KDmodel:
    def __init__(self,numtotprot,numtotlig,data,pconc,lconc,nodelist,header,removeoutliers=False,plot1=None,plot2=None,plot3=None,bootnum=1,maxsites=0,**kwargs):
        ...
        for i in xrange(0,len(self.data[0])):
    ...
    def FindBestModel(self,fixedprotmodel,fixedligmodel):
        ...
        compares=np.array([CompareModels(float(fitssorted[i][0]),float(fitssorted[i+1][0]),float(fitssorted[i][1]),float(fitssorted[i+1][1]),len(np.ravel(self.data)),0.99) for i in xrange(0,len(fits)-1)])
        test=0
        print "P Values: "
        print compares
    ...
    def RunKDFit(self,**kwargs):
        ...
        self.kdargs.kds=[np.mean(self.kdargs.lconc) for i in xrange(0,self.numkd)]
        ...
        if self.maxsites!=0 and self.maxsites is not None:
            ...
            niarr=np.clip(narr-iarr,0,sys.maxint)
            self.kdargs.nfactors=scipy.misc.factorial(narr)/(scipy.misc.factorial(niarr)*scipy.misc.factorial(iarr))
        else:
    ...
    def SetupModel(self,**kwargs):
        ...
        for i in range(1,self.numtotprot+1):
            for j in self.nlig:
                if j+1<=self.numtotlig and [i,j]!=[0,0] and ((j+1<=i*self.maxsites and j<=i*self.maxsites) or self.maxsites==0):
        ...
        self.kdmap=range(0,len(self.kdargs.ureact))
        ...
        if len(self.kdargs.degen>0):
            ...
            self.kdmap=np.concatenate([np.array(self.kdmap),np.arange(self.numkd,self.numkd+len(self.kdargs.degen[:,1]))])

        #Set up graph data for plot
        self.graph=[("P"+str(int(self.reactions[i,0,0]))+"L"+str(int(self.reactions[i,0,1])),"P"+str(int(self.reactions[i,2,0]))+"L"+str(int(self.reactions[i,2,1]))) for i in xrange(0,len(self.reactions))]
        self.ugraph=[("P"+str(int(self.kdargs.ureact[i,0,0]))+"L"+str(int(self.kdargs.ureact[i,0,1])),"P"+str(int(self.kdargs.ureact[i,2,0]))+"L"+str(int(self.kdargs.ureact[i,2,1]))) for i in xrange(0,len(self.kdargs.ureact))]
        self.nodenames= ["P"+str(int(self.nodes[i,0]))+"L"+str(int(self.nodes[i,1])) for i in xrange(0,len(self.nodes))]
    ...
    def ModPaths(self):
        ...
        if(self.ligflag==2):
            for i in xrange(0,len(self.kdargs.paths)):
                if(len(self.kdargs.paths[i])==1 and np.all(self.kdargs.ureact[self.kdargs.paths[i][0],1]==[0,1])):
                    onepath=self.kdargs.paths[i][0]
            for i in xrange(0,len(self.kdargs.paths)):
                for j in xrange(0,len(self.kdargs.paths[i])):
        ...
        if(self.protflag==2):
            for i in xrange(0,len(self.kdargs.paths)):
                if(len(self.kdargs.paths[i])==1 and np.all(self.kdargs.ureact[self.kdargs.paths[i][0],1]==[1,0])):
                    onepath=self.kdargs.paths[i][0]
            for i in xrange(0,len(self.kdargs.paths)):
                for j in xrange(0,len(self.kdargs.paths[i])):
    ...
    def MakeFitGrid(self):
        ...
        out=[MinFree(kdargs.pconc[i],kdargs.lconc[i],kdargs.ureact,kdargs.nprottab,kdargs.nligtab,kdargs.paths,kdargs.kds,kdargs.pfrees[i],kdargs.lfrees[i],kdargs.nfactors) for i in xrange(0,len(kdargs.lconc))]
        out=np.array(out)
        kdargs.pfrees=out[:,0]
        kdargs.lfrees=out[:,1]
        out2=[[MakeGrid(kdargs.pfrees[i],kdargs.lfrees[i],kdargs.ureact,kdargs.nprottab,kdargs.nligtab,kdargs.paths,kdargs.kds,kdargs.nfactors)[0][row[0],row[1]] for row in kdargs.nodelist]  for i in xrange(0,len(kdargs.lconc))]
        self.fitgrid=np.transpose(np.array([safedivide(np.array(out2[i]),np.sum(out2[i])) for i in xrange(0,len(kdargs.lconc))]))
    ...
    def Bootstrap(self,std,numpts):
        ...
        hists=[np.reshape(np.histogram(np.array(np.random.random_integers(0,len(np.ravel(self.data))-1,size=len(np.ravel(self.data)))),bins=range(0,len(np.ravel(self.data))+1))[0],np.shape(self.data)) for i in xrange(0,numpts)]
        ...
        try:
            ...
            [queue.put((self.data,self.kdargs,hists[i])) for i in xrange(0,numpts)]
            workers=[multiprocessing.Process(target=BootMinWorker,args=(queue,results_queue)) for i in xrange(0,multiprocessing.cpu_count())]
            for p in workers:
                p.start()
            count=0
            while True:
    ...
    def PlotHist(self,topax=None):
        if topax is not None and self.randfit is not None:
            ...
            if self.maxsites==0:
                ...
                xlab="Log Micro KD"
            plot.histogram(np.log10(self.randfit.transpose()),labels=self.kdmap,xlab=xlab,ylab="histogram",title="KD Distribution")
...
if __name__=="__main__":
    ...
    if switch==0:
        ...
        lconc=data[1]
        data=data[2:]
        header=string.split(file,".")[0]
        nprot=1
        nlig=4
        maxsites=4
        nodelist=[[1,j] for j in xrange(0,nlig+1)]
    elif switch==1:
        path="C:\\cprog\\UniFit"
        file="trimersim.txt"
        os.chdir(path)
        ...
        file="PeptideBinding1D.txt"
        os.chdir(path)
        data=np.loadtxt(file)
        dims=data.shape
        pconc=[5. for i in xrange(0,dims[1])]
    ...
    fit=KDmodel(nprot,nlig,data,pconc,lconc,nodelist,header,prot="free",lig="parallel",bootnum=0,maxsites=maxsites)
    #fit.RunBootstrap()
