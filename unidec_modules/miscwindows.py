...
class AdditionalParameters(wx.Dialog):
    ...
    def InitUI(self,config):
        ...
        self.inputbox6b = wx.ComboBox(self.pnl, wx.ID_ANY, style=wx.CB_READONLY,choices=["None","Harmonics","Satellites","Both","Total","Ultimate","Isotope"])
...
class SaveFigureDialog(wx.Dialog):
    ...
    def OnClose(self, e):
        ...
        self.rect=[float(self.b1.GetValue()),float(self.b2.GetValue()),float(self.b3.GetValue()),float(self.b4.GetValue())]
    ...
    def on_choose_dir(self,e):
        if self.directory is not None:
            dlg=wx.DirDialog(None,"Choose Top Directory","",wx.DD_DEFAULT_STYLE | wx.DD_DIR_MUST_EXIST)#,defaultPath=self.directory)
        else:
