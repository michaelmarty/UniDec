...
def new_get_data(start, end, im_bin_size, raw_file, pusher, function_no, scan_start, scan_end, dir=None):
    ...
    p = subprocess.call(call_params,
                        shell=False)  # , stdout=devnull, shell=False) # extract arrival time distributions for a set of peaks
...
def new_get_data_MS(start, end, bin_size, raw_file, function_no, scan_start, scan_end, dir=None):
    ...
    p = subprocess.call(
        call_params)  # , shell=False) #, stdout=devnull, shell=False) # extract arrival time distributions for a set of peaks
