...
void z_slice(const float *ivals, const int mindex, float *zdat, const int zlen, const float thresh)
{
	...
	{
		const float val= ivals[index2D(zlen, mindex, i)];
...
void z_slice_range(const float *ivals, const int mindex1, const int mindex2, float *zdat, const int zlen, const float thresh)
{
	for (int i = mindex1; i < mindex2; i++)
	{
		...
		{
			const float val = ivals[index2D(zlen, i, j)];
...
float extract_zmax(Config config, const float peak, const float *xvals, const float *yvals, const float *zvals, const int mlen, const int zlen, const float thresh)
{
	...
	float *zdat = calloc(zlen, sizeof(float));
	if (zdat == NULL) {
		printf("Error allocating memory for zdat.\n");
		return 1;
	}
...
void charge_peak_extracts(int argc, char *argv[], Config config, const int ultra)
{
	...
	if (peakx == NULL) {
		printf("Error allocating memory for peakx.\n");
		exit(1);
	}
	...
	if (extracts == NULL) {
		printf("Error allocating memory for extracts.\n");
		free(peakx);
		exit(1);
	}
	...
	for (int i = 0; i < num; i++)
	{
		...
		if (mass_ints == NULL || massaxis == NULL) {
			printf("Error allocating memory for mass_ints or massaxis.\n");
			free(peakx);
			free(extracts);
			exit(1);
		}
		...
		if (zints == NULL || zaxis == NULL) {
			printf("Error allocating memory for zints or zaxis.\n");
			free(peakx);
			free(extracts);
			free(massaxis);
			exit(1);
